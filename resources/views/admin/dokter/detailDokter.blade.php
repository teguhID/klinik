@extends('admin.layout.layout')

@section('content')
    <h2 class="mb-4">Data "Nama" <a href="{{ url('/editDokter') }}" class="btn btn-success float-right">Edit "Nama"</a></h2>

    <div class="container">
        <div class="row justify-content-md-center">
            <div class="col-4">
                <img src="data:image/png;base64," width="300" height="300">
            </div>
        </div>
    </div>
    <br>
    <div class="container">
        <div class="row justify-content-md-center">
            <div class="col-6" align="right">Nama :</div>
            <div class="col-6" align="left">"Nama"</div>
        </div>
        <div class="row justify-content-md-center">
            <div class="col-6" align="right">No STR :</div>
            <div class="col-6" align="left">"123123123"</div>
        </div>
        <div class="row justify-content-md-center">
            <div class="col-6" align="right">Kualifikasi :</div>
            <div class="col-6" align="left">"Dokter Umum"</div>
        </div>
        <div class="row justify-content-md-center">
            <div class="col-6" align="right">Alamat :</div>
            <div class="col-6" align="left">"Alamat"</div>
        </div>
        <div class="row justify-content-md-center">
            <div class="col-6" align="right">Jenis Kelamin :</div>
            <div class="col-6" align="left">"Pria"</div>
        </div>
        <div class="row justify-content-md-center">
            <div class="col-6" align="right">Universitas :</div>
            <div class="col-6" align="left">"Universitas BSI"</div>
        </div>
        <div class="row justify-content-md-center">
            <div class="col-6" align="right">Kontak :</div>
            <div class="col-6" align="left">"0897 7873 7675"</div>
        </div>
    </div>

    <script>
        $(document).ready(function(){
            $("#dashboardNav").removeClass("active");
            $("#dokterNav").addClass("active");
            $("#penyakitNav").removeClass("active");
            $("#pasienNav").removeClass("active");
            $("#userNav").removeClass("active");
        });
    </script>

@endsection
